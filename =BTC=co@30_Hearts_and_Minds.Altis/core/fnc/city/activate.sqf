...
if (btc_global_reputation < 150) then {
        playSound3d [getMissionPath "core\sounds\katyusha.ogg", _buildings select 0, false, getPosASL (_buildings select 0), 5, 1, 600];
        [[getPos _city, 300], btc_fnc_civ_get_weapons] call btc_fnc_delay_exec;
        [[getPos _city, 300], btc_fnc_civ_get_grenade] call btc_fnc_delay_exec;
        systemChat "teste 1";
};
